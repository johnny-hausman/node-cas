...
CAS.prototype.handleSingleSignout = function(req, res, next, logoutCallback)
{
    if (req.method == 'POST' && req.body['logoutRequest']) {
        ...
        try {
            ...
            var $ = cheerio.load(req.body['logoutRequest'], { xml: true });
  ...
  var req = https.get({
    ...
    res.on('end', function() {
      ...
      // CAS 2.0 (XML response, and extended attributes)
        ...
        var $ = cheerio.load(response, { xml: true });
...
var parseAttributes = function(elemSuccess) 
{
    ...
    if (elemAttribute && elemAttribute.children().length > 0) {
        ...
        for (var i=0; i<elemAttribute.children().length; i++) {
            var child = elemAttribute.children().eq(i);
            var raw = child.get(0);
            if (!raw) continue;
            var attrName = raw.name.toLowerCase().replace(/cas:/, '');
            if (attrName != '#text') {
                var attrValue = child.text();
        ...
        for (var i=0; i<elemSuccess.children().length; i++) {
            var child = elemSuccess.children().eq(i);
            var raw = child.get(0);
            if (!raw) continue;
            var tagName = raw.name.toLowerCase().replace(/cas:/, '');
            switch (tagName) {
                case 'user':
                case 'proxies':
